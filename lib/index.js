...
const init = async (redisConfig, kafkaConfig, producerConfig) => {
    ...
    kafka = new Kafka({
        ...
        connectionTimeout: 3000, // 연결 타임아웃 (밀리초)
        requestTimeout: 25000,   // 요청 타임아웃 (밀리초)
        retry: {
          retries: 10,
        },
            ...
            await consumer.run({
                eachMessage: async ({ topic, partition, message }) => {
                    try {
                        ...
                        if (typeof messageConsumeInfo.headers.data === "string") {
                            try {
                                const data = JSON.parse(messageConsumeInfo.headers.data)
