            ...
            await consumer.run({
                eachMessage: async ({ topic, partition, message }) => {
                    try {
                        ...
                        // 출발지 client id
                        if (headers.departure !== undefined && headers.departure !== null) {
                            messageConsumeInfo.headers.departure = headers.departure.toString()
                        }

                        // 도착지 client id
                        if (headers.arrival !== undefined && headers.arrival !== null) {
                            messageConsumeInfo.headers.arrival = headers.arrival.toString()
                        }
...
const send = async (messageSendInfo, options) => {
    ...
    const value = messageSendInfo.value
...
/**
 * redis & kafka 연결 종료
 * @param {*} callback 작업 완료시 호출되는 콜백 함수
 */
const exit = (callback) => {
    try {
